...
(defun % (command &rest arguments)
  ...
  (shell-command (apply 'concat command (s-join " " arguments))))

(defun %-str (command &rest arguments)
  "Run COMMAND with ARGUMENTS, returning its output as a string."
  (shell-command-to-string (apply 'concat command (s-join " " arguments))))

(defun %-async (command &rest arguments)
  "Run COMMAND with ARGUMENTS, returning its output as a string."
  (async-shell-command (apply 'concat command (s-join " " arguments))))
