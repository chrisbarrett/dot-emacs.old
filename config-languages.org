...
(defun cb-hs:first-word-on-line ()
  (car (s-match (rx bol (* space) (group (+ (not space))))
                (current-line))))
...
(defun cb-hs:meta-ret ()
  ...
  (cond
   ...
   ((s-matches? (rx bol (* space) (+ word) (+ space) "::") (current-line))
    (cb-hs:insert-function-template (cb-hs:first-word-on-line)))

   ;; Insert new pattern match case below the current one.
   ((s-matches? (rx bol (* space) (+ (not (any "="))) "->") (current-line))
    (goto-char (line-end-position))
   ...
   (t
    (let ((fname (cb-hs:first-word-on-line)))
      (cond
       ;; New function case.
       ((and fname
             (or (s-matches? (eval `(rx bol (* space) ,fname (+ space) "::"))
                             (buffer-string))
                 (s-matches? (eval `(rx bol (* space) ,fname (+ nonl) "="))
                             (current-line))))
