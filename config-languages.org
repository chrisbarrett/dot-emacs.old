...
#+begin_src emacs-lisp
(after 'bison-mode
  (define-key bison-mode-map (kbd "=") (smart-op "="))
  (define-key bison-mode-map (kbd ">") 'c-insert-smart-gt))
#+end_src
* Lisp
#+begin_src emacs-lisp
...
(define-key haskell-mode-map (kbd "C-a") 'shm/goto-parent)
(define-key haskell-mode-map (kbd "C-e") 'shm/goto-parent-end)
(define-key haskell-mode-map (kbd "M-t") 'shm/transpose)
(define-key haskell-mode-map (kbd "C-k") 'shm/kill)
#+end_src
*** Use shm for outdenting and indenting
#+begin_src emacs-lisp
(bind-key "S-<tab>" 'shm/simple-indent-backtab haskell-mode-map)
(bind-key "<tab>" 'shm/simple-indent haskell-mode-map)
#+end_src
...
(define-key haskell-mode-map (kbd "=") (command (smart-insert-operator "=")))
(define-key haskell-mode-map (kbd "<") 'cbhs:smart-lt)
(define-key haskell-mode-map (kbd ".") 'cbhs:smart-dot)
(define-key haskell-mode-map (kbd "|") 'cbhs:smart-pipe)
(define-key haskell-mode-map (kbd "?") (smart-op "?"))
(define-key haskell-mode-map (kbd "$") (smart-op "$"))
(define-key haskell-mode-map (kbd ",") (smart-op ","))
(define-key haskell-mode-map (kbd "/") (smart-op "/"))
(define-key haskell-mode-map (kbd "+") (smart-op "+"))
...
(--each (list haskell-mode-map inferior-haskell-mode-map)
  (define-key it (kbd "C-c h") 'hoogle)
  (define-key it (kbd "C-c C-c") 'haskell-process-cabal-build))
#+end_src
...
(defun cb-hs:newline-and-expand-snippet-at-col (predicate col)
...
(defun cb-hs:rx-start-column (rx)
  (save-excursion
    (goto-char (line-end-position))
...
(defun cb-hs:meta-ret ()
  ...
  (cond
   ...
   ;; Insert case after case statement.
   ((s-matches? (rx symbol-start "case" symbol-end) (current-line))
    (goto-char (line-end-position))
    (newline-and-indent)
   ...
   ((s-matches? (rx bol (* space) "|") (current-line))
    (cb-hs:newline-and-insert-at-col (cb-hs:case-start-col) "| "))
   ...
   ((s-matches? (rx bol (* space) (+ nonl) "->") (current-line))
    (cb-hs:newline-and-expand-snippet-at-col
     (C (~ equal "match-case") yas--template-name)
     (cb-hs:case-start-col)))
