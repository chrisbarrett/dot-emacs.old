  ...
  (unless (or (derived-mode-p 'text-mode) (in-string?) (in-comment?))
    (save-restriction
      ...
      (when (s-matches? (concat (regexp-opt smart-op-list) " *$")
        ...
        (when (s-matches? (rx (not (any space)) (+ space) eol)
                          (buffer-substring (line-beginning-position) (point)))
          (delete-horizontal-space))
        t))))
...
* JSON
#+begin_src emacs-lisp
(cb:install-packages-on-buffer-name "\\.json" '(json-mode))
#+end_src
...
* csv-mode
#+begin_src emacs-lisp
(cb:install-packages-on-buffer-name "\\.csv$" '(csv-mode))
#+end_src
...
* Markdown
#+begin_src emacs-lisp
(let ((re (rx "." (or "md" "markdown") eol)))
  (cb:install-packages-on-buffer-name re '(markdown-mode))
  (add-to-list 'auto-mode-alist `(,re . markdown-mode)))
#+end_src
** DEFER
#+begin_src emacs-lisp
(after 'markdown-mode
...
* Clojure
#+begin_src emacs-lisp
(cb:install-packages-on-buffer-name (rx "." (or "clj" "edn" "dtm" "cljs" "cljx"))
  '(clojure-mode
    cider
    ac-nrepl))
#+end_src
...
* Python
#+begin_src emacs-lisp
(cb:install-packages-on-buffer-name "\\.py"
  '(python
    python-info
    jedi
    virualenv))
...
* Ruby
#+begin_src emacs-lisp
(cb:install-packages-on-buffer-name
    (rx (or "Rakefile" "Vagrantfile" "Thorfile" "Capfile" "GuardFile" "Gemfile"
...
(cb:install-packages-on-buffer-name (rx ".ya" (? "ml") eol)
  '(yaml-mode))
#+end_src
** Ignore RBC files
#+begin_src emacs-lisp
...
* Scala
#+begin_src emacs-lisp
(cb:install-packages-on-buffer-name (rx (or ".scala" ".sbt"))
  '(scala-mode2))
#+end_src
** DEFER
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
(cb:install-packages-on-buffer-name
    (rx "." (or "hs" "gs" "hi" "pghci" "cabal" "hsc" "hcr"))
  '(haskell-mode
    ghc
    hi2
    shm))
...
(after 'haskell-mode
#+end_src
** Load inf-haskell for its keymap
#+begin_src emacs-lisp
(require 'inf-haskell)
#+end_src
...
#+end_src
** install hi2 for haskell indentation
#+begin_src emacs-lisp
(hook-fn 'haskell-mode-hook
  (turn-on-hi2)
  (diminish 'hi2-mode))
...
#+end_src
*** Re-implement =shm/:= to use smart operators
#+begin_src emacs-lisp
...
#+end_src
*** Re-implement =shm/close-paren= to use smartparens
#+begin_src emacs-lisp
(defun cb-hs:close-brace ()
  "Insert a closing brace."
  (interactive)
  (when (cb-hs:inside-parens?)
    (sp-insert-or-up ")"))
  (shm/reparse)
  (shm/goto-parent-end))

(define-key shm-map (kbd ")") 'cb-hs:close-brace)
#+end_src
*** Use TAB commands for indentation and moving between slots
#+begin_src emacs-lisp
(defun cb-hs:tab ()
  "Jump to the next shm slot or indent the current line."
  (interactive)
  (or (shm/jump-to-slot) (hi2-indent-line)))

(defun cb-hs:backtab ()
  "Jump to the previous shm slot or backward indent the current line."
  (interactive)
  (or (shm/jump-to-previous-slot) (hi2-indent-backwards)))

(define-key shm-map (kbd "TAB") 'cb-hs:tab)
(define-key shm-map (kbd "<backtab>") 'cb-hs:backtab)
#+end_src
** Configure commands for switching between src and REPL
#+begin_src emacs-lisp
(defadvice switch-to-haskell (after maybe-insert-state activate)
  "Enter insert state if point is at the end of the line."
  (when (s-blank? (buffer-substring (point) (line-end-position)))
    (cb:maybe-evil-insert-state)))
  ...
  (let* ((before (buffer-substring (line-beginning-position) (point)))
    ...
    (cond
     ...
     (t
      (smart-insert-op "#")))))
#+end_src
*** Define a custom smart colon command for GHCI
#+begin_src emacs-lisp
...
(define-key inferior-haskell-mode-map (kbd "-") 'cb-hs:smart-minus)
(define-key inferior-haskell-mode-map (kbd ".") 'cb-hs:smart-dot)
(define-key inferior-haskell-mode-map (kbd "|") 'cb-hs:smart-pipe)
(define-key inferior-haskell-mode-map (kbd ":") 'cb-hs:ghci-smart-colon)
#+end_src
** hideshow
*** Define helper functions
#+begin_src emacs-lisp
...
(defun cb-hs:meta-ret ()
  ...
  (cond
   ...
   (t
    (let ((fname (cb-hs:first-ident-on-line)))
      (cond
       ;; New function case.
       ((and fname
             (or
        ...
        (cb-hs:insert-function-template fname)
        (save-excursion (hi2-indent-line))
       ...
       (t
        ...
        (or (atomic-change-group (shm/newline-indent))
            (hi2-newline-and-indent))
        (message "New line"))))))
...
* Idris
#+begin_src emacs-lisp
(cb:install-packages-on-buffer-name (rx ".idr" eol)
  '(idris-mode))
#+end_src
** ignore .idr files
#+begin_src emacs-lisp
...
* SML
#+begin_src emacs-lisp
(cb:install-packages-on-buffer-name (rx "." (or "sml" "sig" "cm" "grm"))
  '(sml-mode))
...
* OCaml
#+begin_src emacs-lisp
(cb:install-packages-on-buffer-name (rx ".ml" (? (any "y" "i" "l" "p")))
  '(tuareg
    merlin))
#+end_src
** Configure OPAM
...
#+begin_src emacs-lisp
(cb:install-packages-on-buffer-name (rx ".fs" (? (any "i" "y" "l" "x")) eol)
  '(fsharp-mode))
#+end_src
** auto-complete
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
(cb:install-packages-on-buffer-name
    (rx "." (or "tex" "dtx" "ins" "ltx" "sty" "cls" "clo" "bbl" "dry" "lco") eol)
  '(auctex
    latex-preview-pane))
