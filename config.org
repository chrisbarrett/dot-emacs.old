...
(defun cb-eshell:directory-info ()
  (cl-destructuring-bind (perms _links owner group &rest _)
      (s-split (rx (+ space)) (%-string "ls -ld" (%-quote (eshell/pwd))))
    (unless (and
             (f-child-of? (eshell/pwd) user-home-directory)
             (equal owner (%-string "whoami")))
      (list
       (cons "mode" (propertize perms 'face `(:foreground ,solarized-hl-cyan)))
       (cons "owner" (propertize owner 'face `(:foreground ,solarized-hl-cyan)))
       (cons "group" (propertize group 'face `(:foreground ,solarized-hl-cyan)))))))
      ...
      (->> '(("A" "Agenda and next actions"
             ...
             ("W" "Agenda and work actions"
              ((agenda "" ((org-agenda-ndays 14)))
               ...
               (tags-todo "-PeterBarrett-PeterRoberts/NEXT"
                     ((org-agenda-overriding-header "Next Actions")))
               (todo "WAITING"
                     ((org-agenda-overriding-header "Waiting")))
               (todo "LEASED"
                     ((org-agenda-overriding-header "Leased Equipment")))
               (tags-todo "PeterBarrett"
                          ((org-agenda-overriding-header "Peter Barrett")))
               (tags-todo "PeterRoberts"
...
(setq org-todo-keywords
      '((type "AWAITING"    "|" "RECIEVED(!)" "CANCELLED(c@)")
        (type "LEASED(@)"   "|" "RETURNED(!)" "LOST(!)")
        (type "OUTSTANDING" "|" "PAID(!)" "VOID(@)")

        (type "READY" "ORGANISE"
              "|" "COLLECTED(!)" "DISPATCHED(!)" "CANCELLED(c@)")

        (type "CALLBACK" "MESSAGE"
              "|" "HANDLED(h@)" "TRANSFERRED" "NOTIFIED")
...
Do not change habits or repeating todos."
...
#+end_src
** org-drill
*** save buffer without prompting after drill session
#+begin_src emacs-lisp
(setq org-drill-save-buffers-after-drill-sessions-p nil)

(defadvice org-drill (after save-buffer activate)
  (save-buffer))
#+end_src
...
(add-to-list 'org-action-picker-options
             '("r" "Org Drill" org-drill-again :when cb-org:drill-buffer?))
#+end_src
...
(cl-defun cm--org-capture (str kind &optional tags)
...
Declare the message parser. It will use another Emacs process to fetch the title
for webpages.
