...
#+end_src
The function uses the =async= package to perform the installation in the
...
(defun update-packages ()
  ...
  (async-start
#+end_src
The function below is executed in the other Emacs. It is populated with a sane
load path and package variables derived from the parent instance.

The functions to call are obtained using =symbol-function= rather than by name.
This is used to inject the functions' implementations directly into the async
process. By doing so, there is no need to load any of my config files.
#+begin_src emacs-lisp
`(lambda ()
   ,(async-inject-variables "^package-")
   ,(async-inject-variables "^load-path")
   ,(async-inject-variables "^cbpkg")
   ...
   (fset 'growl ,(symbol-function 'growl))
   (fset 'cbpkg:updateable-packages ,(symbol-function 'cbpkg:updateable-packages))
   (fset 'cbpkg:install-packages ,(symbol-function 'cbpkg:install-packages))

   (let ((pkgs (cbpkg:updateable-packages)))
     (cbpkg:install-packages pkgs)
     (length pkgs)))
#+end_src
Once updates are complete, call back into the parent Emacs and show a
...
#+begin_src emacs-lisp
(after 'evil
  (evil-global-set-key 'normal "g o" 'mac-open-dwim)
  (evil-global-set-key 'normal "g O" 'mac-reveal-in-finder))
#+end_src
** END
#+begin_src emacs-lisp
)
...
(defun find-ctag ()
  ...
  (interactive)
  (if (file-exists-p (f-join (cb:tags-project-root) "TAGS"))
      (visit-ctags)
    (build-ctags))
...
#+end_src
** git-gutter-fringe+
Improve git-gutter+ to show git status in the fringe.

This is disabled because there were unresolvable dependencies at the time of
writing.
#+begin_src emacs-lisp
...
#+end_src
* imenu
Provides navigation between sections of the current buffer.
#+begin_src emacs-lisp
;(require 'imenu)
#+end_src
* file-template
...
#+end_src
** Utility functions
Define utility functions used by interactive commands.
...
Commands for finding the bounds of transactions.
...
*** Key binding
#+begin_src emacs-lisp
(define-key ledger-mode-map (kbd "C-c C-.") 'ledger-insert-date)
#+end_src
** Insert transactions
Define a command to insert transaction headers.
*** Impl
#+begin_src emacs-lisp
...
*** Key binding
#+begin_src emacs-lisp
(define-key ledger-mode-map (kbd "C-c C-t") 'ledger-insert-transaction-header)
#+end_src
** Expenses
Define a command to interactively insert an expense.
*** Impl
#+begin_src emacs-lisp
...
*** Key binding
#+begin_src emacs-lisp
(define-key ledger-mode-map (kbd "C-c C-e") 'ledger-add-expense)
#+end_src
** Buffer formatting
