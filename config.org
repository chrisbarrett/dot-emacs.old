...
(defun sp-ocaml-just-one-space (id action ctx)
  ...
  (when (and (equal 'insert action)
    ...
    ;; 3. this form is prededed by a `?`, as in a let binding.
    (save-excursion
      (search-backward id)
      (unless (s-matches?
               (rx (or (group bol (* space))
...
(defun cb-ocaml:smart-colon ()
  ...
  (interactive)
  (if (thing-at-point-looking-at (rx "~" (+ word)))
      (insert ":")
    (smart-insert-operator ":")))
...
(defun cb-ocaml:smart-insert-operator (op)
  ...
  (if (thing-at-point-looking-at (rx (any "(" ")")))
      (insert op)
    (smart-insert-operator op)))
...
(defun cb-ocaml:at-let-binding? ()
  (and (s-matches? (rx bol (* space) "let" symbol-end) (current-line))
       (or (not (s-matches? "=" (current-line)))
...
#+begin_src emacs-lisp
(defun cb-ocaml:new-case-at-col (col)
  "Insert case on a new line at COL."
  (goto-char (line-end-position))
  (newline)
...
(defun cb-ocaml:newline-insert-case ()
  "Insert a case template below the current line."
  (goto-char (line-end-position))
  (newline-and-indent)
  (forward-char -2)
