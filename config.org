...
#+end_src
*** Define command to generate org drill cards from tables
org-drill requires individual headlines with the "drill" tag; creating these can
be laborious and it is difficult to get an overview of your cards when the
buffer is folded. Define a command to generate drills from org-mode tables.
#+begin_src emacs-lisp
...
(defun cb-org:goto-table-in-subtree ()
    ...
    (save-excursion
      (save-restriction
        ...
        (-when-let (bounds (org-get-property-block))
          ...
          (forward-line))
        (when (s-matches? "SCHEDULED" (current-line))
          (forward-line))
...
(defun cb-org:existing-cards ()
  "Parse the Cards subtree for existing drill cards.
Return a list of OrgDrillCard."
  (save-excursion
    (when (cb-org:goto-cards)
      (save-restriction
        (org-narrow-to-subtree)
        (let (acc)
          (while (outline-next-heading)
            (setq acc (cons (cb-org:subtree->card) acc)))
          (nreverse acc))))))

(defun cb-org:goto-or-insert-cards-heading ()
...
(defun cb-org:drill-table-tree->OrgDrillCards (heading type instructions)
  "Convert the drill-table tree at point to a list of OrgDrillCards. "
  (let ((rows (save-excursion
                (unless (org-at-table-p)
                  (cb-org:goto-table-in-subtree))
                (cb-org:drill-table-rows))))
    (--map (OrgDrillCard heading type instructions it) rows)))


(defun cb-org:get-or-read-prop (name read-fn)
...
(defun org-drill-generate-from-table (heading type instructions)
...
INSTRUCTIONS is a string describing how to use the card."
  ...
  (let* ((cards (cb-org:drill-table-tree->OrgDrillCards heading
                                                        type
                                                        instructions))
         (existing (cb-org:existing-cards))
         (new-cards (-difference cards existing)))
    (save-excursion
      (cb-org:goto-or-insert-cards-heading )
      (goto-char (line-end-position))
    ...
    (let ((len (length new-cards)))
      ...
      (if (zerop len)
        ...
        (message "Inserted %s new card%s"
                 ...
                 (if (= 1 len) "" "s"))))))
#+end_src
*** Add the above generate command to org-mode's C-c C-c actions.
#+begin_src emacs-lisp
(add-hook 'org-ctrl-c-ctrl-c-hook
          (lambda ()
            (when (and (org-at-table-p)
                       (org-entry-get (point) "DRILL_HEADING"))
              (org-drill-generate-from-table))))
#+end_src
