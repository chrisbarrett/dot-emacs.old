...
(defun cb-ocaml:set-keys (keymap)
  ...
  (--each smart-operator-alist
    (define-key keymap (kbd it) (eval `(smart-op ,it))))
  ...
  (cond
   ...
   ((s-matches? (rx symbol-start "match" symbol-end) (current-line))
    (cb-ocaml:new-case-at-col
     (save-excursion
       (goto-char (line-beginning-position))
       (search-forward-regexp (rx symbol-start "match" symbol-end))
   ...
   ((s-matches? (rx bol (* space) "type") (current-line))
    (cb-ocaml:new-case-at-col
     (save-excursion
       (search-backward-regexp (rx bol (* space) (group "type")))
       (goto-char (match-beginning 1))
       (+ 2 (current-column)))))
   ;; Insert new case below the current one.
