...
(defun cb-org:set-to-at-work (file)
...
FILE is the file to use as the notes file while at work."
  ...
  (cond
   ((equal org-default-notes-file org-init-notes-file)
    (cb-org:set-to-at-work file))
   (t
    (with-current-buffer (find-file-noselect file)
      (cb-org:goto-or-create-work-log)
      (org-clock-out nil t))
...
(defun cb-org:maybe-set-to-work ()
  ...
  (when (f-exists? cb-org:work-persistence-file)
    (cb-org:set-to-at-work org-work-file)))
...
#+begin_src emacs-lisp
(defun cb-org:find-caller-names ()
  "Find the caller names in the current buffer."
  (->> (buffer-string)
    s-no-props
    (s-match-strings-all
     (rx bol (* space) "-" (+ space) (or "from" "to") (+ space) "::"
         (+ space) (group (not (any "%")) (+ nonl)) (* space) eol))
    (-map 'cadr)))

(defun cb-org:read-caller (prompt)
  "Read a caller name, offering completions based on the current buffer."
  (with-current-buffer (find-file-noselect org-default-notes-file)
    (save-restriction
      (widen)
      (ido-completing-read prompt (cb-org:find-caller-names)))))
...
(-each `(("t" "Todo" entry
         ...
         ("p" "Phone Call" entry
          (file+datetree org-default-notes-file)
          ,(s-unlines
            "* TRANSFERRED Call %U  :@phone:"
            "- From :: %(cb-org:read-caller \"From: \")"
            "- To :: %(cb-org:read-caller \"To: \")"
            "- Subject :: %^{Subject}"
            "- Action :: "
            "** Notes"
            "%?")
      ...
      (->> '(("A" "Agenda and next actions"
             ...
             ("W" "Agenda and work actions"
              ((agenda "" ((org-agenda-ndays 14)))
               (todo "READY"
                     ((org-agenda-overriding-header "Collections")))
               (tags-todo "@phone"
...
(setq org-todo-keywords
      '((type "READY(r)" "|" "COLLECTED(c!)" "CANCELLED(k@)")
        (type "LEASED(l@)" "|" "RETURNED(!)" "LOST(!)")
        (type "CALLBACK" "MESSAGE" "|" "TRANSFERRED" "HANDLED(h@)")
        (type "MAYBE(m)" "TODO(t)" "NEXT(n)" "WAITING(w@/!)" "APPT(a)"
...
#+end_src
* Idris
Load =idris-mode= aggressively so redefinitions hold.
#+begin_src emacs-lisp
(cb:install-package 'idris-mode t)
#+end_src
