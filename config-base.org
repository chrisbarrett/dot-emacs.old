...
(global-set-key (kbd "C-x c") 'toggle-centred-mode)
#+end_src
...
#+end_src
** Evil-exchange
Adds commands to swap text from one region to another. Currently disabled
because it raises an error on autoload.
#+begin_src emacs-lisp
...
(setq-default
 ...
 `(
   ...
   ;; Pomodoro
   ;;
   ;; Show the current pomodoro time and an indicator for the length of the next
   ;; break.
   (:eval
    (if (true? org-pomodoro-mode-line)
   ...
   (:eval
    (if (and (true? org-init-notes-file)
...
** Custom find file command that tiles frames
#+begin_src emacs-lisp
